...
function buildPrompt(questions) {
  ...
  return `你是数值分析老师，正在批改作业。注意：题目中可能有填空框，学生答案是填入空缺处的内容，题目中已印出的部分不需要学生重复填写，批改时请结合题目上下文判断学生答案是否正确。对每题输出一行，必须以 ===题N=== 开头（N为题号），然后给出批改：正确给【✓】（若有值得补充的要点可加一句，否则只输出【✓】）；部分正确给【△】并指出缺失点；错误给【✗】并直接给出正确思路。数学含义正确即为正确，忽略符号写法差异。\n\n${items}`;
}
